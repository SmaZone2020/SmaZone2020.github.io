...
function ProjectCard({ project }: ProjectCardProps) {
    return (
        ...
        >
            <Card className="bg-white/40 dark:bg-surface/50 backdrop-blur-sm h-full overflow-hidden rounded-2xl hover:shadow-lg hover:-translate-y-0.5 transition-all duration-200 flex flex-col">
                ...
                <Card.Content className="p-4 flex flex-col flex-1">
                    <div className="flex items-center justify-between mb-2">
                        <h3 className="font-semibold text-base line-clamp-1">
                            {project.title}
                        </h3>
                        <ArrowUpRightFromSquare className="w-3.5 h-3.5 text-gray-400 dark:text-gray-500 opacity-0 group-hover:opacity-100 transition-opacity shrink-0" />
                    </div>

                    <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-2 min-h-[2.5rem] mb-3">
                        {project.description}
                    </p>
